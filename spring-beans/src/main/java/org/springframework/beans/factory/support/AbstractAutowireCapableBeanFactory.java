...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		for (BeanPostProcessor beanProcessor : getBeanPostProcessors()) {
			// @PostConstruct注解在这里处理：InitDestroyAnnotationBeanPostProcessor
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					// 修改 BeanDefinition，在实例化之后，依赖注入之前。
		...
		if (earlySingletonExposure) {
			...
			// AOP 织入
			// 提前将创建的 bean 实例加入到 singletonFactories(第三级缓存) 中
	...
	/**
	 * 提早暴露的 bean 必须是经过动态代理的，否则暴露出去的是原始对象。
	 * <p>
	 * AOP 织入发生在这里，，因此 singletonFactories 这个第三级缓存是必须的。
	 * <p>
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 由编译器生成的，在源代码中没有出现的，都会被标记为 synthetic 。
		// 给予 InstantiationAwareBeanPostProcessor 最后一次机会去修改 bean
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
		...
		// 如果前面的任一 InstantiationAwareBeanPostProcessor 发出停止填充命令，则终止后续操作。
		...
		if (mbd.getResolvedAutowireMode() == RootBeanDefinition.AUTOWIRE_BY_NAME ||
			...
			// 根据属性名称注入(需要有setter方法)
			// 1.@Bean(autowire = BY_NAME)
			// 2.<bean ... autowire="byName" />
			// 根据setter方法解析beanName，而不是参数名称，需要注意。
			// Add property values based on autowire by name if applicable.
			if (mbd.getResolvedAutowireMode() == RootBeanDefinition.AUTOWIRE_BY_NAME) {
				autowireByName(beanName, mbd, bw, newPvs);
			}

			// 根据属性类型注入(需要有setter方法)
			// 1.@Bean(autowire = BY_TYPE)
			// 2.<bean ... autowire="byType" />
		...
		if (hasInstAwareBpps || needsDepCheck) {
			if (pvs == null) {
				// 这里可能是手动赋的值：MergedBeanDefinitionPostProcessor
				// 如果你手动给某个属性赋过值的话，那么Spring在处理@Autowire等注解的时候就不会重复为该属性赋值了。
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					...
					// 解析待注入的值，有可能是数组、列表这种 bean 集合。
					// 将解析出来的所有依赖的 beanName 放到 autowiredBeanNames 中
